...
parser.add_argument("--dataset", type=str, default="/root/autodl-tmp/mamba_s/data_preprocessing/BSDS_DIR", help="where the filtered dataset is stored")
parser.add_argument("--dump_root", type=str, default="/root/autodl-tmp/mamba_s/data_preprocessing/DUMP_DIR", help="Where to dump the data")
parser.add_argument("--b_filter", type=bool, default=False, help="we do not use this in our paper")
parser.add_argument("--num_threads", type=int, default=4, help="number of threads to use")
args = parser.parse_args()

'''
Extract each pair of image and label (including semantic label and edge_label) from .mat to generagte the data for TRAINING and VALIDATION
...
def generate_edge(input, edge_width = 2):

    label = input.squeeze()
    image = np.zeros(label.shape)
#    print(label.shape)
    rows = image.shape[0]
    cols = image.shape[1]
    for row in range(rows):
       for col in range(cols):
          if row + 1 < rows:
             if label[row+1,col]!=label[row,col]:
                 image[row,col]=1
          if col + 1 < cols:
             if label[row,col+1] != label[row,col]:
                 image[row,col]=1
...
def BSD_loader(path_imgs, path_label, b_filter=False):
    ...
    for t in range(len(gtseg_all['groundTruth'][0])):
        ...
        gtedge = gtseg_all['groundTruth'][0][t][0][0][1]
        ...
        if H_ == 321 and W_ == 481:
            gtedge = gtedge[:320, :480]
        elif H_ == 481 and W_ == 321:
            gtedge = gtedge[:480, :320]
        gtedge_lst.append(gtedge)
    edge = gtedge_lst[0].copy()
    for i in range(1,len(gtedge_lst)):
        edge = edge + gtedge_lst[i]
    
    edge = (edge * 51).astype(np.uint8)
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (2, 2))
    edge = cv2.dilate(edge, kernel)

    return img, gtseg_lst, edge
...
def dump_example(n, n_total, dataType, img_path):
    ...
    img, label_lst, edge = BSD_loader(img_path, img_path.replace('images', 'groundTruth')[:-4]+'.mat', b_filter=args.b_filter)
    ...
    for k, label in enumerate(label_lst):
        ...
        dump_edge_file = os.path.join(dump_dir, '{0}_{1}_edge.png' .format(img_name, k))
        cv2.imwrite(dump_edge_file, edge.astype(np.uint8))
