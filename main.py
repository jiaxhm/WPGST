...
import models
import datasets
from loss import compute_semantic_pos_loss,cross_entropy_loss_edge, focal_cross_entropy_loss_edge, dice_loss
import datetime
from tensorboardX import SummaryWriter
from train_util import *
# from models.dynamic_tanh import *
'''
...
parser.add_argument('--arch', '-a', metavar='ARCH', default='lformerr3',  help='model architecture')#GEncoder1   LEncoder1
...
parser.add_argument('--pretrained', dest='pretrained', default=None, help='path to pre-trained model')#'D:\Code\Local_Transformer\CKPT\\ablation\LocalFormer_0823_test_botten_renew_loss\BSD500\lformerr3_adam_3000000epochs_epochSize6000_b8_lr0.0002_posW0.003_25_08_23_17_48\checkpoint.tar'
...
def train(train_loader, model, optimizer, epoch, train_writer, init_spixl_map_idx, xy_feat):
    ...
    for i, (input, label, edge) in enumerate(train_loader):
        ...
        output, pred_edge, pred_edge2 = model(input_gpu)
        ...
        edge_loss = cross_entropy_loss_edge(pred_edge2, edge.to(device))

        loss = slic_loss + edge_loss #0.5 * edge_loss #+ edge_loss2
        # ========= back propagate ===============
        optimizer.zero_grad()
